...
public class Torder extends JFrame {
	...
	// 버튼 클릭 횟수
		...
		static JPanel cartPanel = new JPanel(); // 장바구니 레이블을 추가s할 패널
		static JPanel payPanel = new JPanel();
		static JLabel[] cartLabels = new JLabel[mainName.length]; // 장바구니 레이블 배열
		static JLabel[] payLabels = new JLabel[mainName.length*4]; // 결제 레이블 배열
...
//			pf.repaint();
		...
		// 결체창에서 메뉴 레이블 생성 메서드  //mainName[i], mainPrice[i], mainCnt[i], (mainCnt[i] * mainPrice[i])
		public static void updatePayLabel(JLabel totalLabel) {
			
			int totalPrice = 0;
			
			payPanel.removeAll();
			payPanel.setLayout(new BoxLayout(payPanel, BoxLayout.Y_AXIS));  // 세로로 쌓기
	        

			payPanel.setLayout(new GridLayout(0, 4)); // 4열로 지정
			...
			for (int i = 0; i < mainCnt.length; i++) {
				if (mainCnt[i] > 0) { // 메인 메뉴가 장바구니에 추가된 경우만
					 ...
					 // 공백 추가
		            String nameSpace = "<html>"+ space + mainName[i] + "</html>";
		            String priceSpace = "<html>"+ space + mainPrice[i]  + "원" + "</html>";
		            String cntSpace = "<html>"+ space + mainCnt[i] + "개" + "</html>";
		            String totalSpace = "<html>"+ space + (mainCnt[i] * mainPrice[i])  + "원"+ "</html>";

					// 상품명 레이블 생성 및 추가
					JLabel nameLabel = new JLabel(nameSpace);
					nameLabel.setForeground(Color.WHITE);
					nameLabel.setFont(labelFont);
					payPanel.add(nameLabel); // 패널에 추가

					// 가격 레이블 생성 및 추가
					JLabel priceLabel = new JLabel(priceSpace);
					priceLabel.setForeground(Color.WHITE);
					priceLabel.setFont(labelFont);
					payPanel.add(priceLabel); // 패널에 추가

					// 수량 레이블 생성 및 추가
					JLabel cntLabel = new JLabel(cntSpace);
					cntLabel.setForeground(Color.WHITE);
					cntLabel.setFont(labelFont);
					payPanel.add(cntLabel); // 패널에 추가

					// 총금액 레이블 생성 및 추가
					JLabel totalPLabel = new JLabel(totalSpace);
					totalPLabel.setForeground(Color.WHITE);
					totalPLabel.setFont(labelFont);
					payPanel.add(totalPLabel); // 패널에 추가

					totalPrice += mainPrice[i] * mainCnt[i]; // 총 가격 계산
				}
			}
